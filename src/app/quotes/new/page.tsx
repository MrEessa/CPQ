...
export default function NewQuotePage() {
  ...
  function liveEstimate(products: Product[]): number {
    return products.reduce((sum, p) => sum + calculateCost({ product: p, annualUsageKwh: annualUsage }).subtotal, 0);
  }
    ...
    <div className="max-w-3xl space-y-5">
      ...
      {step === 2 && (
        <div className="space-y-3">
          <div className="rounded-md px-4 py-2 text-xs" style={{ background: 'var(--color-primary-subtle)', color: 'var(--color-primary-text)' }}>
            Showing active products for <strong>{market}</strong> market. Ineligible products are greyed out.
          </div>
          ...
          {activeProducts.map((product) => {
            ...
            const breakdown   = selected ? calculateCost({ product, annualUsageKwh: annualUsage }) : null;
            ...
            return (
              <div
                ...
                style={{
                  ...
                  opacity: !eligibility.eligible ? 0.5 : 1,
              ...
              >
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-2">
                    <input type="checkbox" checked={selected} readOnly disabled={!eligibility.eligible} className="mt-0.5" />
                    <div>
                  ...
                  {breakdown && (
                    <div className="text-right">
                      ...
                      <p className="font-semibold" style={{ color: 'var(--color-primary-text)', fontFamily: 'var(--font-mono)' }}>{formatCurrency(breakdown.subtotal)}</p>
                    </div>
                  )}
                </div>

                {!eligibility.eligible && (
                  <div className="mt-2 flex items-start gap-1.5 rounded-md px-2.5 py-1.5 text-xs" style={{ background: 'var(--color-warning-subtle)', color: 'var(--color-warning-text)' }}>
                ...
                {selected && breakdown && (
                  <div className="mt-3 grid grid-cols-3 gap-2 pt-3 text-xs" style={{ borderTop: '1px solid var(--border-subtle)', color: 'var(--text-secondary)' }}>
                    <div>
                      <p style={{ color: 'var(--text-tertiary)' }}>Standing charge</p>
                      <p className="font-medium" style={{ fontFamily: 'var(--font-mono)' }}>{product.pricingStructure.standingCharge !== undefined ? formatStandingCharge(product.pricingStructure.standingCharge) : '—'}</p>
                    </div>
        ...
        <div className="space-y-4">
          <Card>
            ...
            <div className="grid grid-cols-3 gap-2 text-sm">
              {[['Name', customerName], ['Type', customerType], ['Annual Usage', `${annualUsage.toLocaleString()} kWh`]].map(([label, val]) => (
                <div key={label}>
                  ...
                  <tbody>
                    {bd.standingChargeAnnual > 0 && (
                      <tr style={{ borderBottom: '1px solid var(--border-subtle)' }}>
                        ...
                        <td className="py-1 text-right" style={{ color: 'var(--text-primary)', fontFamily: 'var(--font-mono)' }}>{formatCurrency(bd.standingChargeAnnual)}</td>
                      </tr>
                    )}
                    {bd.rateLines.map((line) => (
                      <tr key={line.label} style={{ borderBottom: '1px solid var(--border-subtle)' }}>
                        ...
                        <td className="py-1 text-right" style={{ color: 'var(--text-primary)', fontFamily: 'var(--font-mono)' }}>{formatCurrency(line.cost)}</td>
                      </tr>
                    ))}
                    {bd.leviesTotal > 0 && (
                      <tr style={{ borderBottom: '1px solid var(--border-subtle)' }}>
                        <td className="py-1" style={{ color: 'var(--text-secondary)' }}>Levies</td>
                        <td className="py-1 text-right" style={{ color: 'var(--text-primary)', fontFamily: 'var(--font-mono)' }}>{formatCurrency(bd.leviesTotal)}</td>
                      </tr>
                    )}
                    <tr style={{ borderTop: '1px solid var(--border-default)' }}>
                      <td className="py-1.5 font-medium" style={{ color: 'var(--text-primary)' }}>Subtotal (ex VAT)</td>
                      <td className="py-1.5 text-right font-medium" style={{ color: 'var(--text-primary)', fontFamily: 'var(--font-mono)' }}>{formatCurrency(bd.subtotal)}</td>
                    </tr>
                    <tr>
                      <td className="py-1" style={{ color: 'var(--text-secondary)' }}>VAT ({bd.vat > 0 ? ((bd.vat / bd.subtotal) * 100).toFixed(0) : 0}%)</td>
                      <td className="py-1 text-right" style={{ color: 'var(--text-secondary)', fontFamily: 'var(--font-mono)' }}>{formatCurrency(bd.vat)}</td>
                    </tr>
                    <tr style={{ background: 'var(--color-primary-subtle)' }}>
                      ...
                      <td className="rounded-r px-2 py-1.5 text-right font-semibold" style={{ color: 'var(--color-primary-text)', fontFamily: 'var(--font-mono)' }}>{formatCurrency(bd.total)}</td>
                    </tr>
                  </tbody>
                </table>
              </Card>
